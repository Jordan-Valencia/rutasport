...
UPDATE order_items
   SET price = CAST(REPLACE(REPLACE(REPLACE(REPLACE(price, ',', ''), '.', ''), '$', ''), ' ', '') AS INTEGER)
 WHERE price IS NOT NULL;
ALTER TABLE orders RENAME COLUMN wompi_transaction_id TO epayco_refpayco;
...
-- Add `slogan` column to heroes table (editable subtitle "LLEVA MÁS LEJOS.")
ALTER TABLE heroes ADD COLUMN slogan TEXT;
