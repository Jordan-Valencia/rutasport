...
// ../.wrangler/tmp/bundle-gmTbnu/checked-fetch.js
...
var onRequestPut7 = /* @__PURE__ */ __name(async ({ env, request, params }) => {
  ...
  try {
    const b = await request.json();
    if (!b.name?.trim() || !b.price?.trim()) return json7({ error: "name y price son requeridos" }, 400);
    const result = await env.DB.prepare(
...
var onRequestPost5 = /* @__PURE__ */ __name(async ({ env, request }) => {
  ...
  try {
    const b = await request.json();
    if (!b.name?.trim() || !b.price?.trim()) return json17({ error: "name y price son requeridos" }, 400);
...
var QUERY = `
  ...
  GROUP BY p.id
`;
var onRequestGet16 = /* @__PURE__ */ __name(async ({ env, params }) => {
  try {
    const id = params.id;
    if (!id || isNaN(+id)) {
...
var onRequestGet17 = /* @__PURE__ */ __name(async ({ env }) => {
  try {
    const result = await env.DB.prepare(
...
var onRequestPost14 = /* @__PURE__ */ __name(async ({ env, request }) => {
  try {
    ...
    const totalCOP = body.items.reduce((acc, item) => {
      const pesos = parseInt(item.price.replace(/[^\d]/g, "")) || 0;
      return acc + pesos * item.quantity;
    }, 0);
    ...
    const stmts = body.items.map(
      (item) => env.DB.prepare(
        `INSERT INTO order_items (order_id, product_id, name, brand, model, size, price, quantity)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(order.id, item.productId, item.name, item.brand ?? null, item.model ?? null, item.size ?? null, item.price, item.quantity)
    );
...
var onRequestGet24 = /* @__PURE__ */ __name(async ({ env, params }) => {
  try {
...
// ../.wrangler/tmp/pages-xWrX4E/functionsRoutes-0.5035487555085489.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-gmTbnu/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-gmTbnu/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9168230446055142.mjs.map
