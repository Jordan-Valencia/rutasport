...
// .wrangler/tmp/bundle-JY18k7/checked-fetch.js
...
// .wrangler/tmp/pages-AL6krv/functionsWorker-0.47320182372968533.mjs
...
var ADMIN_KEY22 = "1663017721@";
var json22 = /* @__PURE__ */ __name2((data, status = 200) => new Response(JSON.stringify(data), { status, headers: { "Content-Type": "application/json" } }), "json");
var unauthorized22 = /* @__PURE__ */ __name2(() => json22({ error: "Unauthorized" }, 401), "unauthorized");
...
var CORS_OPTIONS = new Response(null, {
  headers: {
    "Access-Control-Allow-Origin": "*",
...
var onRequestPost12 = /* @__PURE__ */ __name2(async ({ env, request }) => {
  try {
    ...
    return jsonOk({ message: "Sesi\xF3n cerrada" });
  } catch (e) {
    return jsonErr(e.message ?? "Error interno", 500);
  }
}, "onRequestPost");
...
var onRequestGet17 = /* @__PURE__ */ __name2(async ({ env, request }) => {
...
var onRequestPost14 = /* @__PURE__ */ __name2(async ({ env, request }) => {
  try {
    ...
    const order = await env.DB.prepare("SELECT id FROM orders WHERE reference = ?").bind(reference).first();
    await env.DB.prepare("UPDATE orders SET status = ?, wompi_transaction_id = ? WHERE reference = ?").bind(status, id, reference).run();
    if (order && (status === "DECLINED" || status === "VOIDED" || status === "ERROR")) {
      ...
      for (const item of items.results) {
        ...
        await env.DB.prepare("UPDATE product_inventory SET stock = stock + ? WHERE product_id = ? AND size = ?").bind(item.quantity, item.product_id, rawSize).run();
      }
    }
...
var QUERY = `
  ...
  GROUP BY p.id
`;
var onRequestGet18 = /* @__PURE__ */ __name2(async ({ env, params }) => {
  try {
    const id = params.id;
    if (!id || isNaN(+id)) {
...
var onRequestPost15 = /* @__PURE__ */ __name2(async ({ env, request }) => {
  try {
    const body = await request.json();
    if (!body.items?.length) {
      return new Response(JSON.stringify({ error: "Carrito vac\xEDo" }), { status: 400, headers });
    }
    let userId = null;
    const authHeader = request.headers.get("Authorization") ?? "";
    const token = authHeader.startsWith("Bearer ") ? authHeader.slice(7) : null;
    if (token) {
      const session = await env.DB.prepare(`SELECT user_id FROM user_sessions WHERE token = ? AND datetime(expires_at) > datetime('now')`).bind(token).first();
    ...
    for (const item of body.items) {
      const rawSize = item.size ? item.size.replace(/^US/i, "") : null;
      if (!rawSize) continue;
      await env.DB.prepare(`UPDATE product_inventory SET stock = stock - ?
                  WHERE product_id = ? AND size = ? AND stock >= ?`).bind(item.quantity, item.productId, rawSize, item.quantity).run();
    }
    ...
    const order = await env.DB.prepare(`INSERT INTO orders (reference, status, total_in_cents, user_id) VALUES (?, 'PENDING', ?, ?) RETURNING id`).bind(reference, amountInCents, userId).first();
    ...
    const signature = await sha256hex2(`${reference}${amountInCents}COP${env.WOMPI_INTEGRITY_KEY}`);
...
var onRequestGet24 = /* @__PURE__ */ __name2(async ({ env, request }) => {
  try {
    const url = new URL(request.url);
    ...
    return new Response(JSON.stringify(result.results), { headers: headers2 });
  } catch (e) {
    return new Response(
...
var onRequestGet25 = /* @__PURE__ */ __name2(async ({ env }) => {
  try {
    const result = await env.DB.prepare(
...
var onRequestGet26 = /* @__PURE__ */ __name2(async ({ env, params }) => {
  try {
    ...
    obj.writeHttpMetadata(headers3);
    if (!headers3.get("content-type")) {
      const ext = key.split(".").pop()?.toLowerCase() ?? "";
      headers3.set("content-type", MIME_TYPES[ext] ?? "application/octet-stream");
    }
    headers3.set("cache-control", "public, max-age=31536000, immutable");
    return new Response(obj.body, { headers: headers3 });
  } catch (e) {
...
var onRequestGet27 = /* @__PURE__ */ __name2(async ({ env, params, request }) => {
  try {
    ...
    if (!headers3.get("content-type")) {
      const ext = key.split(".").pop()?.toLowerCase() ?? "";
      headers3.set("content-type", MIMES[ext] ?? "video/mp4");
    }
    headers3.set("accept-ranges", "bytes");
    headers3.set("cache-control", "public, max-age=31536000, immutable");
    if (rangeHeader && obj.range) {
      const r = obj.range;
      const offset = r.offset ?? 0;
      const length = r.length ?? obj.size - offset;
      headers3.set("content-range", `bytes ${offset}-${offset + length - 1}/${obj.size}`);
      headers3.set("content-length", String(length));
      return new Response(obj.body, { status: 206, headers: headers3 });
    }
    headers3.set("content-length", String(obj.size));
    return new Response(obj.body, { headers: headers3 });
  } catch {
    return new Response("Internal server error", { status: 500 });
  }
...
// .wrangler/tmp/bundle-JY18k7/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-JY18k7/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.47320182372968533.js.map
