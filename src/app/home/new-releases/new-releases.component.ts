...
export class NewReleasesComponent {
  ...
  private async animateCards() {
    ...
    if (heading)      gsap.from(heading, { scrollTrigger: { trigger: section, start: 'top 82%' }, y: 30, autoAlpha: 0, duration: 0.7, ease: 'power3.out' })
    if (cards.length) gsap.from(cards,   { scrollTrigger: { trigger: section, start: 'top 78%' }, x: 60, autoAlpha: 0, duration: 0.6, stagger: 0.12, ease: 'power3.out' })
  }
}
