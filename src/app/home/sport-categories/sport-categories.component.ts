...
export class SportCategoriesComponent {
  ...
  protected filteredProducts = computed(() => {
    ...
    return products.filter(p => p.sport === sport || p.category === sport)
  })
  ...
  private async animateSection() {
    ...
    if (h2)          gsap.from(h2,   { scrollTrigger: { trigger: section, start: 'top 82%' }, y: 30, autoAlpha: 0, duration: 0.7, ease: 'power3.out' })
    if (btns.length) gsap.from(btns, { scrollTrigger: { trigger: section, start: 'top 78%' }, y: 20, autoAlpha: 0, duration: 0.5, stagger: 0.08, ease: 'power3.out' })
    this.animateProductsIn()
  }
  ...
  private async animateProductsIn() {
    ...
    if (cards.length) gsap.from(cards, { y: 40, autoAlpha: 0, duration: 0.5, stagger: 0.08, ease: 'power3.out', clearProps: 'all' })
  }
}
