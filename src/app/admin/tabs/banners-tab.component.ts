...
@Component({
  ...
  templateUrl: './banners-tab.component.html',
})
export class BannersTabComponent implements OnInit {
  ...
  async load() {
    ...
    this.apiError.set('')
    try {
      const res = await this.svc.apiFetch('/api/admin/banners')
  ...
  openAdd() {
    ...
    this.formData = { title: '', subtitle: '', description: '', image: '', buttonText: 'VER MÁS', bgColor: '#0f172a', order: 0, isActive: true }
    this.formErrors = {}
  ...
  async uploadImage(event: Event) {
    ...
    try {
      ...
      if (res.ok) { const { path } = await res.json(); this.formData = { ...this.formData, image: path } }
  ...
  async save() {
    ...
    try {
      const res = mode === 'add'
        ? await this.svc.apiFetch('/api/admin/banners',       { method: 'POST', body: JSON.stringify(data) })
        : await this.svc.apiFetch(`/api/admin/banners/${id}`, { method: 'PUT',  body: JSON.stringify(data) })
  ...
  async deleteItem(id: number) {
    ...
    this.apiError.set('')
    try {
      const res = await this.svc.apiFetch(`/api/admin/banners/${id}`, { method: 'DELETE' })
      if (!res.ok) { const err = await res.json().catch(() => ({})) as any; this.apiError.set(err.error ?? `Error ${res.status}`); return }
