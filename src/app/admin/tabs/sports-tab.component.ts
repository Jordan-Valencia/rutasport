...
export class SportsTabComponent implements OnInit {
  ...
  async save() {
    ...
    try {
      const res = mode === 'add'
        ? await this.svc.apiFetch('/api/admin/sports',       { method: 'POST', body: JSON.stringify(data) })
        : await this.svc.apiFetch(`/api/admin/sports/${id}`, { method: 'PUT',  body: JSON.stringify(data) })
  ...
  async deleteItem(id: number) {
    ...
    this.apiError.set('')
    try {
      const res = await this.svc.apiFetch(`/api/admin/sports/${id}`, { method: 'DELETE' })
      if (!res.ok) { const err = await res.json().catch(() => ({})) as any; this.apiError.set(err.error ?? `Error ${res.status}`); return }
